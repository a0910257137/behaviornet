...
def run(bfm):
    ...
    sp = np.random.rand(n_sp, 1)
    ep = np.random.rand(n_ep, 1)
    ...
    for angle_i in range(13):
        ...
        transformed_vertices = 0.00095 * vertices.dot(R.T)
        image_vertices = transform.to_image(transformed_vertices, [96, 96],
                                            h=192)
