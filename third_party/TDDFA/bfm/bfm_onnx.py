...
def _load_tri(bfm_fp):
    if osp.split(bfm_fp)[-1] == 'bfm_noneck_v3.pkl':
        tri = _load(make_abs_path('../configs/tri.pkl')
...
class BFMModel_ONNX(nn.Module):
    ...
    def __init__(self, bfm_fp, shape_dim=40, exp_dim=10):
        ...
        w_shp = _to_tensor(
            bfm.get('w_shp').astype(np.float32)[..., :shape_dim])
...
def convert_bfm_to_onnx(bfm_onnx_fp, shape_dim=40, exp_dim=10):
    ...
    bfm_decoder = BFMModel_ONNX(bfm_fp=bfm_fp,
                                shape_dim=shape_dim,
                                exp_dim=exp_dim)
    bfm_decoder.eval()

    # dummy_input = torch.randn(12 + shape_dim + exp_dim)
    dummy_input = torch.randn(3, 3), torch.randn(3, 1), torch.randn(
        shape_dim, 1), torch.randn(exp_dim, 1)
    R, offset, alpha_shp, alpha_exp = dummy_input
    torch.onnx.export(bfm_decoder, (R, offset, alpha_shp, alpha_exp),
                      bfm_onnx_fp,
