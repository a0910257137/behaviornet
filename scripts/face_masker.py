...
class PRN:
    ...
    def __init__(self, model_path, data_root):
        ...
        self.face_ind = np.loadtxt(
            os.path.join(data_root, "uv-data/face_ind.txt")).astype(np.int32)
        self.triangles = np.loadtxt(
            os.path.join(data_root, "uv-data/triangles.txt")).astype(np.int32)
        # self.face_ind = np.loadtxt('Data/uv-data/triangles.txt').astype(
        #     np.int32)
        # self.triangles = np.loadtxt('Data/uv-data/triangles.txt').astype(
        #     np.int32)
        self.net = PRNet(3, 3)
    ...
    def process(self, image, image_info):
        ...
        src_pts = np.array([[center[0] - size / 2, center[1] - size / 2],
                            ...
                            [center[0] + size / 2, center[1] - size / 2]])
        DST_PTS = np.array([[0, 0], [0, self.resolution - 1],
                            [self.resolution - 1, 0]])
        tform = estimate_transform('similarity', src_pts, DST_PTS)
        cropped_image = warp(image,
                             tform.inverse,
                             output_shape=(self.resolution, self.resolution))
        cropped_image = np.transpose(cropped_image[np.newaxis, :, :, :],
        ...
        cropped_pos = np.transpose(cropped_pos,
                                   (0, 2, 3, 1)).squeeze() * self.MaxPos
        # restore
...
class FaceMasker:
    ...
    """
    def __init__(self, is_aug, data_root, model_root):
        """init for FaceMasker
        ...
        self.uv_face_path = os.path.join(data_root, "uv-data/uv_face_mask.png")
        # self.uv_face_path = 'Data/uv-data/uv_face_mask.png'
        # self.mask_template_folder = 'Data/mask-data'
        # self.prn = PRN('model/prnet.pth')

        self.mask_template_folder = os.path.join(data_root, "mask-data")
        self.prn = PRN(os.path.join(model_root, "prnet.pth"), data_root)
    ...
    def get_ref_texture_src(self):
        ...
        for mask_template in mask_template_list:
            mask_template_path = os.path.join(self.mask_template_folder,
                                              mask_template)
    ...
    def add_mask(self, face_root, image_name2lms, image_name2template_name,
                 masked_face_root):
        for image_name, face_lms in image_name2lms.items():
            ...
            self.add_mask_one(image_path, face_lms, template_name,
                              masked_face_path)
    ...
    def add_mask_one(self, image, face_lms, template_name):
        ...
        if c == 4:
            image = image[:, :, :3]
        pos, vertices = self.get_vertices(
            face_lms, image)  #3d reconstruction -> get texture.
        image = image / 255.  #!!
        texture = cv2.remap(image,
                            ...
                            borderValue=(0))
        new_texture = self.get_new_texture(ref_texture_src, uv_mask_src,
                                           texture)
        new_colors = self.prn.get_colors_from_texture(new_texture)

        # render
        face_mask, new_image = render_cy(
        ...
        valid = np.asarray(np.where(face_mask == True))
        flag = True if valid.shape[1] != 0 else False
    ...
    def get_vertices(self, face_lms, image):
        ...
        Args:
            ...
            image:[0, 255]
        """
        lms_infos = np.asarray(face_lms)
        valid_n = np.all(np.isfinite(lms_infos), axis=-1)

        lms_info = np.reshape(lms_infos[valid_n], (-1, 68, 2))

        n, c, d = np.shape(lms_info)
        idx = random.randint(0, n - 1)
        lms_info = lms_info[idx]
    ...
    def get_new_texture(self, ref_texture_src, uv_mask_src, texture):
        ...
        if self.is_aug:
            ...
            if np.random.rand() > 0.5:
                ...
                ref_texture_temp[y_offset:256 - y_offset,
                                 x_offset:256 - x_offset, :] = cv2.resize(
                                     ref_texture, target_size)
                uv_mask_temp[y_offset:256 - y_offset,
                             x_offset:256 - x_offset] = cv2.resize(
                                 uv_mask, target_size)
            ...
            if np.random.rand() > 0.5:
                ...
                alpha = (alpha_r[1] -
                         alpha_r[0]) * np.random.rand() + alpha_r[0]
                beta = beta
                img = ref_texture * 255
        ...
        new_texture = texture * (
            1 - uv_mask[:, :, np.newaxis]
        ) + ref_texture[:, :, :3] * uv_mask[:, :, np.newaxis]
        return new_texture
