...
class Conv2d(nn.Module):
    def __init__(self,
        ...
        self.padding_num = stride * (output_size -
                                     1) - input_size + kernel_size
        self.even_conv = nn.Conv2d(in_channel,
                                   out_channel,
        ...
        self.odd_conv = nn.Sequential(
            nn.ConstantPad2d(
                (self.padding_num // 2, self.padding_num // 2 + 1,
                 self.padding_num // 2, self.padding_num // 2 + 1), 0),
...
class UpBlock(nn.Module):
    def __init__(self,
        ...
        self.upConvTranspose = nn.ConvTranspose2d(in_channels,
                                                  out_channels,
                                                  kernel_size=4,
                                                  stride=2,
                                                  padding=1,
...
class PRNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        ...
        self.encoder_block_1 = ResBlock(size,
                                        size * 2,
                                        kernel_size=4,
                                        stride=2,
                                        input_size=256)  # 128x128x32
        self.encoder_block_2 = ResBlock(size * 2,
                                        size * 2,
                                        kernel_size=4,
                                        stride=1,
                                        input_size=128)  # 128x128x32
        self.encoder_block_3 = ResBlock(size * 2,
                                        size * 4,
                                        kernel_size=4,
                                        stride=2,
                                        input_size=128)  # 64x64x64
        self.encoder_block_4 = ResBlock(size * 4,
                                        size * 4,
                                        kernel_size=4,
                                        stride=1,
                                        input_size=64)  # 64x64x64
        self.encoder_block_5 = ResBlock(size * 4,
                                        size * 8,
                                        kernel_size=4,
                                        stride=2,
                                        input_size=64)  # 32x32x128
        self.encoder_block_6 = ResBlock(size * 8,
                                        size * 8,
                                        kernel_size=4,
                                        stride=1,
                                        input_size=32)  # 32x32x128
        self.encoder_block_7 = ResBlock(size * 8,
                                        size * 16,
                                        kernel_size=4,
                                        stride=2,
                                        input_size=32)  # 16x16x256
        self.encoder_block_8 = ResBlock(size * 16,
                                        size * 16,
                                        kernel_size=4,
                                        stride=1,
                                        input_size=16)  # 16x16x256
        self.encoder_block_9 = ResBlock(size * 16,
                                        size * 32,
                                        kernel_size=4,
                                        stride=2,
                                        input_size=16)  # 8x8x512
        self.encoder_block_10 = ResBlock(size * 32,
                                         size * 32,
                                         kernel_size=4,
                                         stride=1,
                                         input_size=8)  # 8x8x512

        self.decoder_block_1 = UpBlock(size * 32, size * 32,
                                       stride=1)  # 8x8x512
        self.decoder_block_2 = UpBlock(size * 32, size * 16,
                                       stride=2)  # 16x16x256
        self.decoder_block_3 = UpBlock(size * 16, size * 16,
                                       stride=1)  # 16x16x256
        self.decoder_block_4 = UpBlock(size * 16, size * 16,
                                       stride=1)  # 16x16x256
        self.decoder_block_5 = UpBlock(size * 16, size * 8,
                                       stride=2)  # 32 x 32 x 128
        self.decoder_block_6 = UpBlock(size * 8, size * 8,
                                       stride=1)  # 32 x 32 x 128
        self.decoder_block_7 = UpBlock(size * 8, size * 8,
                                       stride=1)  # 32 x 32 x 128
        self.decoder_block_8 = UpBlock(size * 8, size * 4,
                                       stride=2)  # 64 x 64 x 64
        self.decoder_block_9 = UpBlock(size * 4, size * 4,
                                       stride=1)  # 64 x 64 x 64
        self.decoder_block_10 = UpBlock(size * 4, size * 4,
                                        stride=1)  # 64 x 64 x 64

        self.decoder_block_11 = UpBlock(size * 4, size * 2,
                                        stride=2)  # 128 x 128 x 32
        self.decoder_block_12 = UpBlock(size * 2, size * 2,
                                        stride=1)  # 128 x 128 x 32
        self.decoder_block_13 = UpBlock(size * 2, size,
        ...
        self.decoder_block_17 = UpBlock(3,
                                        3,
                                        stride=1,
