...
def map_texture(src_image,
    ...
    for y in range(h):
        for x in range(w):
            ...
            #     w0 = w1 = w2 = 1./3
            # src
            src_texel = w0 * src_vertices[:2,
                                          tri[0]] + w1 * src_vertices[:2, tri[
                                              1]] + w2 * src_vertices[:2,
                                                                      tri[2]]  #
            #
            if src_texel[0] < 0 or src_texel[0] > sw - 1 or src_texel[
                    1] < 0 or src_texel[1] > sh - 1:
                dst_image[y, x, :] = 0
                continue
            ...
            if mapping_type == 'nearest':
                dst_image[y, x, :] = src_image[int(round(src_texel[1])),
                                               int(round(src_texel[0])), :]
            # bilinear
            elif mapping_type == 'bilinear':
                # next 4 pixels
                ul = src_image[int(np.floor(src_texel[1])),
                               int(np.floor(src_texel[0])), :]
                ur = src_image[int(np.floor(src_texel[1])),
                               int(np.ceil(src_texel[0])), :]
                dl = src_image[int(np.ceil(src_texel[1])),
                               int(np.floor(src_texel[0])), :]
                dr = src_image[int(np.ceil(src_texel[1])),
                               int(np.ceil(src_texel[0])), :]

                yd = src_texel[1] - np.floor(src_texel[1])
                xd = src_texel[0] - np.floor(src_texel[0])
                dst_image[y, x, :] = ul * (1 - xd) * (1 - yd) + ur * xd * (
                    1 - yd) + dl * (1 - xd) * yd + dr * xd * yd
    ...
    for i in range(triangles.shape[1]):
        ...
        for u in range(umin, umax + 1):
            for v in range(vmin, vmax + 1):
                ...
                if tri_depth[i] > depth_buffer[
                        v, u]:  # and is_pointIntri([u,v], vertices[:2, tri]):
                    depth_buffer[v, u] = tri_depth[i]
...
def get_triangle_buffer(vertices, triangles, h, w):
    ...
    triangle_buffer = np.zeros_like(
        depth_buffer,
        dtype=np.int32) - 1  # if -1, the pixel has no triangle correspondance
...
def vis_of_vertices(vertices, triangles, h, w, depth_buffer=None):
    ...
    for i in range(vertices.shape[1]):
        ...
        if np.floor(vertex[0]) < 0 or np.ceil(vertex[0]) > w - 1 or np.floor(
                vertex[1]) < 0 or np.ceil(vertex[1]) > h - 1:
            continue
